...
@interface ExploreViewController () <UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate>
...
- (void) makeConnetion:(id)data
{
    ...
    NSURL *url = [NSURL URLWithString:@"http://localhost:8080/api/api-auth/login/"];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    NSString *authStr = [NSString stringWithFormat:@"%@:%@", @"Sermilion", @"1"];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                                               ...
                                               NSData *data, NSError *connectionError)
     {
         ...
         if (data.length > 0 && connectionError == nil)
         {
             ...
             article.title = [[greeting objectForKey:@"title"] stringValue];
             NSLog(@"Got response form server: %@", [[greeting objectForKey:@"title"] stringValue]);
